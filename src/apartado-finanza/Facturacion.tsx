...
export default function Facturacion({
  ...
  const handleGenerateInvoice = (order: Order) => {
    ...
    order.items.forEach((item) => {
      ...
      doc.text(formatCOP(item.price), 145, y, { align: 'center' });

      const itemSubtotal = item.price * item.quantity;
      doc.text(formatCOP(itemSubtotal), 180, y, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(formatCOP(subtotalBruto), 180, y, { align: 'center' });

    y += 5;
    doc.setFont('helvetica', 'bold');
    doc.text('IMPUESTO IMPOCONSUMO (8%):', 120, y);
    doc.setFont('helvetica', 'normal');
    doc.text(formatCOP(taxAmount), 180, y, { align: 'center' });
    ...
    doc.text(formatCOP(finalTotal), 180, y + 1, { align: 'center' });
    ...
    doc.text('Esta factura ha sido procesada mediante la Mesa de Caja y Finanzas de Cafe Pandora.', 105, y + 5, { align: 'center' });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-between items-center pb-2.5 border-b border-pandora-wood/20">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Comandas preparadas que están listas para cobrar.</p>
        </div>
      ...
      {billingOrders.length === 0 ? (
        <div className="py-12 bg-white rounded-xl border-2 border-dashed border-slate-300 p-6 text-center select-none">
          ...
          <p className="text-[10px] text-slate-500 mt-1">No hay comandas preparadas esperando facturación.</p>
        </div>
      ) : (
                  ...
                  {!isBillFormOpen ? (
                    <>
                      <button
                        ...
                        className="py-1.5 px-2 rounded-lg bg-[#2E7D32] hover:bg-[#25632a] text-white transition-all text-[10px] font-mono font-bold flex items-center justify-center gap-1 cursor-pointer shadow-sm uppercase tracking-wider"
                      >
                      ...
                      <button
                        ...
                        className="py-1.5 px-2 rounded-lg border border-rose-800 bg-[#3a1a1a]/65 hover:bg-rose-950 text-rose-200 transition-all text-[10px] font-mono font-bold flex items-center justify-center gap-1 cursor-pointer shadow-xs uppercase tracking-wider"
                      >
                        Cancelar
                      </button>
                    </>
