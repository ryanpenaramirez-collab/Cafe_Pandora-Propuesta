    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-sm bg-[#F5F9FF] rounded-2xl shadow-2xl relative border-t-8 border-[#C8A96E] max-h-[90vh] flex flex-col"
        >
          <button
            onClick={onClose}
            className="absolute top-3 right-3 p-1.5 hover:bg-slate-200 rounded-full text-[#8AAAC8] cursor-pointer transition-colors z-10"
          >
            <X className="w-4 h-4" />
          </button>
            ...
            {printType === 'cooking' ? (
              <>
                <div className="text-center pb-3 border-b border-dashed border-[#D0E8F8]">
                  ...
                  <h5 className="font-serif font-black text-sm tracking-widest text-[#2C3E55] uppercase leading-none">
                    PANDORA COMANDA
                  </h5>
                </div>

                <div className="py-3 space-y-1.5 text-xs text-[#5A7A9A] font-mono border-b border-dashed border-[#D0E8F8]">
                ...
                <div className="py-3">
                  <div className="grid grid-cols-12 text-[10px] font-bold uppercase text-[#8AAAC8] pb-2 border-b select-none">
                  ...
                  <div className="divide-y divide-dotted pt-2">
                    {combinedItems.map((item, idx) => (
                      <div key={idx} className="grid grid-cols-12 text-sm py-2">
                        <span className="col-span-9 font-serif font-bold text-[#2C3E55]">{item.name}</span>
                ...
                <div className="text-center pt-3 border-t border-dashed border-[#D0E8F8] text-[10px] text-[#8AAAC8] italic">
              ...
              <>
                <div className="text-center pb-3 border-b border-dashed border-[#D0E8F8]">
                  <span className="text-[10px] bg-[#E8F5EE] text-emerald-800 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider mb-2 inline-block">
                    FACTURA POS
                  </span>
                  <h5 className="font-serif font-black text-sm tracking-widest text-[#2C3E55] uppercase leading-none">
                    CAFÉ PANDORA
                  </h5>
                  <p className="text-[9px] text-[#8AAAC8] mt-1">Dirección local · NIT 901.381.189-4</p>
                </div>
                ...
                <div className="py-3 space-y-1.5 text-xs text-[#5A7A9A] font-mono border-b border-dashed border-[#D0E8F8]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold uppercase text-[10px] bg-[#D0E8F8] px-1.5 py-0.5 rounded">{paymentMethod}</span>
                  </div>
                </div>

                <div className="py-3">
                  <div className="grid grid-cols-12 text-[9px] font-bold uppercase text-[#8AAAC8] pb-2 border-b border-dashed select-none">
                  ...
                  <div className="space-y-1.5 pt-2">
                    {combinedItems.map((item, idx) => (
                      <div key={idx} className="grid grid-cols-12 text-xs items-center text-[#2C3E55]">
                ...
                <div className="border-t border-dashed border-[#D0E8F8] py-3 text-xs font-mono space-y-1">
                  ...
                  <div className="flex justify-between text-lg font-extrabold text-[#2C3E55] border-t border-dotted pt-2 mt-2">
                    <span>TOTAL CLIENTE:</span>
                    <span className="text-[#5BA882]">{formatCOP(total)}</span>
                  </div>
                </div>

                <div className="text-center pt-2 border-t border-dashed border-[#D0E8F8] text-[10px] text-[#8AAAC8] space-y-1">
                  ...
                  <button
                    onClick={onToggleTax}
                    className="mt-2 px-3 py-1 bg-[#E2EDF7] hover:bg-slate-300 text-[#5A7A9A] rounded-lg text-[9px] font-bold cursor-pointer transition-colors"
                  >
